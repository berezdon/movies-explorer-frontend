  ...
  return (
    <section className={`header ${!isMain && 'header_white'}`}>
      <NavLink to='/' className={`header__img ${!isMain && 'header__img_open'}`}>
        <img onClick={handleClickHome} className="logo" src={logoHeader} alt={"Логотип в шапке сайта"} />
      </NavLink>
      ...
      <ul className={`header__nav-moves ${isLogin && 'header__menu header__nav-moves_open'}`}>
